      ...
      {listing && !loading && !error && (
        <>
          <Carousel showThumbs={false} className="relative">
            {listing.imageUrls.map((image, ind) => (
              <div key={ind} className="h-[650px] sm:h-[450px]">
                <img
                  src={image}
                  alt={`Image ${ind + 1}`}
                  className="flex items-center justify-center h-[650px] sm:h-[450px]"
                />
              </div>
            ))}
          </Carousel>
          ...
          <div className="flex flex-col max-w-4xl mx-auto p-3 my-7 gap-4">
            ...
            {listing && currentUser._id !== listing.userRef && !contact && (
              <button
                ...
                className="bg-slate-700 text-white p-3 rounded-lg uppercase mt-8"
              >
                Contact Landowner
              </button>
            )}
